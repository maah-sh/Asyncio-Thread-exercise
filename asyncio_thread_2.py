...
async def async_output_line(input_line):
    return await asyncio.to_thread(get_output_line,input_line)
...
async def main(input_path, output_path):
    ...
    async with aiofiles.open(output_path, mode='w') as output_file:
        output_lines = await asyncio.gather(*(async_output_line(line) for line in input_lines))
